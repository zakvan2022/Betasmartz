(function($) {
  $.fn.listswap = function(options) {   
    ...
    var controls = '<div class="listbox_controls">' + 
                    '<ul>' +
                      '<li class="add btn btn-default">' + settings.label_add + '</li>' +
                      '<li class="remove btn btn-default">' + settings.label_remove + '</li>' +
                    '</ul>' +
                   '</div>';
    ...
    $(div_id + ' .listbox_controls .add').click(function() {
      ...
      refresh_list();
      $('#' + destination_select_id).trigger('optionsChanged');
    });
